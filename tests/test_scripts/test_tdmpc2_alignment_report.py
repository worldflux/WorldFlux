...
def _payload(
    *,
    model_id: str,
    model_profile: str,
    canonical_compare_profile: str | None = "proof_5m",
    ...
    return {
        ...
        "metrics": {
            ...
            "metadata": {
                ...
                "canonical_compare_profile": canonical_compare_profile,
                "required_metrics": required_metrics or ["final_return_mean", "auc_return"],
...
def test_tdmpc2_alignment_report_marks_mismatched_when_canonical_compare_profile_differs(
    tmp_path: Path,
) -> None:
    root = _repo_root()
    official = tmp_path / "official.json"
...
def test_tdmpc2_alignment_report_marks_mismatched_when_required_metric_missing(
    tmp_path: Path,
) -> None:
    root = _repo_root()
    official = tmp_path / "official.json"
    ...
    output = tmp_path / "alignment.json"
    official.write_text(
        json.dumps(_payload(model_id="tdmpc2:5m", model_profile="5m")),
        encoding="utf-8",
    )
    payload = _payload(model_id="tdmpc2:proof_5m", model_profile="proof_5m")
    del payload["metrics"]["auc_return"]
    worldflux.write_text(json.dumps(payload), encoding="utf-8")
