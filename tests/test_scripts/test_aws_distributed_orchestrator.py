...
def test_aws_distributed_orchestrator_wait_mode_with_mocked_aws(
    ...
    def fake_run_local_script(script: Path, args: list[str], *, check: bool = True):  # noqa: ARG001
...
def test_aws_distributed_orchestrator_emits_rerun_manifest_on_missing_pairs(
    ...
    def fake_run_cli(command: list[str]) -> subprocess.CompletedProcess[str]:
        if command[:3] == ["aws", "ssm", "send-command"]:
            return subprocess.CompletedProcess(command, 0, stdout="cmd-1\n", stderr="")
        if command[:3] == ["aws", "ssm", "get-command-invocation"]:
            payload = {
                "Status": "Success",
                "ResponseCode": 0,
        ...
        if command[:3] == ["aws", "s3", "cp"]:
            ...
            if src.startswith("s3://"):
                ...
                if target.name == "parity_runs.jsonl":
                    ...
                    target.write_text(
                        json.dumps({"seeds": [0], "selected_tasks": ["atari100k_pong"]}),
                        encoding="utf-8",
                    )
            else:
                # local -> s3 uploads are expected to succeed
                if not Path(src).exists():
                    return subprocess.CompletedProcess(
                        command, 1, stdout="", stderr="missing source"
                    )
            return subprocess.CompletedProcess(command, 0, stdout="", stderr="")
        return subprocess.CompletedProcess(command, 0, stdout="", stderr="")

    def fake_run_local_script(script: Path, args: list[str], *, check: bool = True):
        if script.name == "merge_parity_runs.py":
            output = Path(args[args.index("--output") + 1])
            summary = Path(args[args.index("--summary-output") + 1])
            inputs = [
        ...
        if script.name == "validate_matrix_completeness.py":
            out = Path(args[args.index("--output") + 1])
            rerun = Path(args[args.index("--rerun-manifest-output") + 1])
            out.parent.mkdir(parents=True, exist_ok=True)
            out.write_text(
                json.dumps({"missing_pairs": 1, "pass": False, "rerun_manifest": str(rerun)}),
                encoding="utf-8",
            )
            rerun.write_text(
                json.dumps({"tasks": [{"task_id": "atari100k_pong"}]}), encoding="utf-8"
            )
            return subprocess.CompletedProcess([str(script), *args], 1, stdout="", stderr="")
    ...
    summary_path = output_dir / "parity_test_missing" / "orchestrator_summary.json"
    summary = json.loads(summary_path.read_text(encoding="utf-8"))
    assert summary["coverage"]["pass"] is False
    assert summary["coverage"]["missing_pairs"] == 1
    assert Path(summary["artifacts"]["rerun_manifest"]).exists()
