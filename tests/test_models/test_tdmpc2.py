...
from worldflux import AutoWorldModel, Batch, TDMPC2Config
...
class TestTDMPC2WorldModel:
    ...
    def test_simnorm_constraint(self, small_model):
        ...
        reshaped = state.tensors["latent"].view(4, -1, 8)
    ...
    def test_encode(self, small_model):
        ...
        assert state.meta.get("latent_type") == "simnorm"
    ...
    def test_predict(self, small_model):
        ...
        assert next_state.tensors["latent"].shape == state.tensors["latent"].shape
    ...
    def test_observe(self, small_model):
        ...
        assert torch.allclose(next_state.tensors["latent"], direct_encode.tensors["latent"])
    ...
    def test_decode_no_obs(self, small_model):
        ...
        assert "obs" not in decoded.preds
        assert "reward" in decoded.preds
        assert "q_values" in decoded.preds
        assert "action" in decoded.preds
    ...
    def test_loss(self, small_model):
        ...
        assert "consistency" in loss_out.components
        assert "reward" in loss_out.components
        assert "td" in loss_out.components
