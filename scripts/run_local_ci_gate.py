...
def build_gate_commands() -> tuple[GateCommand, ...]:
    ...
    return (
        ...
        GateCommand(
            name="Pytest (3.10)",
            argv=("uv", "run", "--python", "3.10", "pytest", "tests/", "-v", "--tb=short"),
        ),
        GateCommand(
            name="Pytest (3.11)",
            argv=("uv", "run", "--python", "3.11", "pytest", "tests/", "-v", "--tb=short"),
        ),
        GateCommand(
            name="Pytest (3.12)",
            argv=("uv", "run", "--python", "3.12", "pytest", "tests/", "-v", "--tb=short"),
        ),
        GateCommand(
        ...
        GateCommand(
            ...
            argv=("uv", "run", "pytest", "-q", "tests/test_integration/test_legacy_bridge_v02.py"),
        ),
        GateCommand(
            name="Build docs (strict)",
            argv=("uv", "run", "mkdocs", "build", "--strict"),
        ),
        _lychee_command(),
        GateCommand(
