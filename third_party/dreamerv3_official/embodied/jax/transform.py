...
def init(
    ...
    params_sharding, grouping = resolve_rules(params_shapes, param_partition_rules, mesh)
    if print_partition:
            ...
            if re.search(rule, name):
                ...
                def apply(y):
                    ...
                    if not hasattr(type(y), "tracer_shardings"):
                        type(y).tracer_sharding = property(lambda self: TRACER_SHARDINGS[id(self)])
...
def resolve_rules(params, partition_rules, mesh):
    ...
    sharding = jax.tree.map(lambda spec: jax.sharding.NamedSharding(mesh, spec), params_spec)
