...
from . import internal, transform
...
class Agent(embodied.Agent):
    ...
    def __init__(self, model, obs_space, act_space, config, jaxcfg):
        ...
        devices = np.array(available).reshape(jax.process_count(), jax.local_device_count())
        ...
        self.policy_sharded = jax.sharding.NamedSharding(self.policy_mesh, P(("d", "f")))
        self.policy_mirrored = jax.sharding.NamedSharding(self.policy_mesh, P())
        self.train_mesh = internal.mesh(self.train_devices, self.jaxcfg.train_mesh, ("d", "f", "t"))
        ...
        self.partition_rules = getattr(self.model, "partition_rules", ([(".*", P())], []))
        ...
        if self.jaxcfg.enable_policy:
            ...
            self.policy_params = internal.move(policy_params, self.policy_params_sharding)
        ...
        self._ckpt_groups = internal.grouped_ckpt_fns(self.params, self.jaxcfg.ckpt_chunksize)
    ...
    def init_train(self, batch_size):
        ...
        return self._init_train(self.params, self._seeds(0, self.train_mirrored), batch_size)

    def init_report(self, batch_size):
        batch_size = batch_size * jax.process_count()
        if self.jaxcfg.use_shardmap:
            batch_size = batch_size // self.train_mesh.size
        return self._init_report(self.params, self._seeds(0, self.train_mirrored), batch_size)
    ...
    def train(self, carry, data):
        ...
        with self.train_lock:
            with elements.timer.section("jit_train"):
                with jax.profiler.StepTraceAnnotation("train", step_num=int(self.n_updates)):
                    self.params, carry, outs, mets = self._train(dona, allo, seed, carry, data)
