...
def combined(
    ...
    if not args.remote_replay:
        workers.append(
            portal.Process(parallel_replay, make_replay_train, make_replay_eval, make_stream, args)
        )
...
def parallel_learner(agent, barrier, args):
    ...
    if args.from_checkpoint:
        elements.checkpoint.load(
            args.from_checkpoint, dict(agent=bind(agent.load, regex=args.from_checkpoint_regex))
        )
    cp.load_or_save()
    ...
    stream_train = iter(agent.stream(embodied.streams.Stateless(parallel_stream("train"))))
    stream_report = iter(agent.stream(embodied.streams.Stateless(parallel_stream("report"))))
    stream_eval = iter(agent.stream(embodied.streams.Stateless(parallel_stream("eval"))))
