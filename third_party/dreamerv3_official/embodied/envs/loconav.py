...
class LocoNav(embodied.Env):
    ...
    def __init__(
        self, name, repeat=1, size=(64, 64), camera=-1, again=False, termination=False, weaker=1.0
    ):
        if name.endswith("hz"):
        ...
        env = composer.Environment(
            time_limit=60, task=task, random_state=None, strip_singleton_obs_buffer_dim=True
        )
        from . import dmc
