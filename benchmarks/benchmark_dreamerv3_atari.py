#!/usr/bin/env python3
"""DreamerV3 benchmark with quick and full modes."""
...
def _load_atari_buffer(path: str) -> ReplayBuffer:
    ...
    obs = data["obs"]
    actions = data["actions_onehot"]
    rewards = data["rewards"]
    dones = data["dones"]

    buffer = ReplayBuffer(
        capacity=len(obs), obs_shape=tuple(obs.shape[1:]), action_dim=int(actions.shape[1])
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="DreamerV3 benchmark")
    add_common_cli(parser, default_output_dir="outputs/benchmarks/dreamerv3-atari")
    parser.add_argument("--data", type=str, default="atari_data.npz")
    args = parser.parse_args()

    mode = resolve_mode(args)
    context = build_run_context(scenario="benchmark_dreamerv3_atari", mode=mode)
    ...
    try:
        if mode == "quick":
            ...
            buffer = _load_atari_buffer(args.data)
            total_steps = 40
            batch_size = 16
            seq_len = 32

        model = create_world_model(
            "dreamerv3:ci", obs_shape=buffer.obs_shape, action_dim=buffer.action_dim
        )
        trainer = Trainer(
            model,
            TrainingConfig(
        ...
        horizon = 12 if mode == "quick" else 24
