...
`tdmpc2:proof_5m` is the canonical WorldFlux profile for proof-grade official
comparison. The proof pipeline expects:

- official system: `tdmpc2:5m`
- WorldFlux system: `tdmpc2:proof_5m`
- backend profile: `proof_5m`

The alignment report generated by `scripts/parity/tdmpc2_alignment_report.py`
is the recipe-level reference for that compare path.
